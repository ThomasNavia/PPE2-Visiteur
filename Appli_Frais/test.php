...
function activerBD($idCnx) {
    ...
    $ok = mysqli_select_db(connecterServeurBD(),$bd);
    return $ok;
}
...
function deconnecterServeurBD($idCnx) {
    $mysqli_close = mysqli_close($idCnx);
    return $mysqli_close ;
}
...
function obtenirDetailFicheFrais($idCnx, $unMois, $unIdVisiteur) {
    $unMoisB = filtrerChainePourBD($unMois);
    $ligne = false;
    ...
    where idVisiteur='" . $unIdVisiteur . "' and mois='" . $unMoisB . "'";
...
function verifierInfosConnexion($idCnx, $unLogin, $unMdp) {
    $unLoginB = filtrerChainePourBD($unLogin);
    $unMdpB = filtrerChainePourBD($unMdp);
    // le mot de passe est crypt� dans la base avec la fonction de hachage md5
    $req = "select id, nom, prenom, login, mdp from Visiteur where login='".$unLoginB."' and mdp='" . $unMdpB . "'";
